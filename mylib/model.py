import numpy as np
#This model describes the interaction between cytotoxic T cells and tumor cells.
# Tumor cells must accumulate 'nhits' sequential hits from T cells before dying.
# Hit delivery does NOT cause T cell detachment — the T cell remains bound
# after each hit and continues delivering hits until the threshold is reached.



# hit delivery does not cause the detachment
def main_model(C, t,params):
    ...
    for hit_idx in range(nhits+1):
        if hit_idx==0:
            derivs.append(u0*(C[hit_idx]+1*C[(nhits+1)+hit_idx])*(1-totalcell/Kmax)   -(k1)*(C[hit_idx])*C[-1]    +     k2*C[(nhits+1)+hit_idx])
...
#### not used in the simulation or analysis   #############################################################
